...
const int c_nColorSubItemCount = 16;
...
public:
	...
	COLORREF clrText;
	COLORREF clrBk;
	COLORREF clrSub16ItemsText[c_nColorSubItemCount];
};
...
int CReportCtrl::InsertItem(int nIndex, LPCTSTR pszText, ...)
{
	...
	for(int iColumn = 1; iColumn < GetColumnCount(); iColumn++)
	{
		if (!bfinish)
		{
			lpsz = va_arg(maker, LPCTSTR);
			if (lpsz == NULL || !AfxIsValidString(lpsz))
			{
				lpsz = NULL;
				bfinish = TRUE;
			}
...
BOOL CReportCtrl::DeleteAllListItems()
{
...
BOOL CReportCtrl::_IsHeadNumber(LPCTSTR pszText)
{
	const int LEN = _tcslen(pszText);
...
int CALLBACK CReportCtrl::_CompareFunction(LPARAM lParam1, LPARAM lParam2, LPARAM lParamData)
{
	...
	if(_IsNumber(s1) && _IsNumber(s2))
	{
		return pListCtrl->m_wndHeader.IsSortAscending() ? _NumberCompare(s1, s2) : _NumberCompare(s2, s1);
	}
	else if (_IsHeadNumber(s1) && _IsHeadNumber(s2))
	{
		return pListCtrl->m_wndHeader.IsSortAscending() ? _NumberCompare(s1, s2) : _NumberCompare(s2, s1);
	}
//	else if(_IsDate(s1) && _IsDate(s2))
...
BOOL CReportCtrl::_AssignNewItemData(int iItem, LPCTSTR lpItem)
{
	...
	pid->bUseUserClr = FALSE;
	pid->clrText = ::GetSysColor(COLOR_WINDOWTEXT);
	pid->clrBk = ::GetSysColor(COLOR_WINDOW);

	for (i = 0; i < c_nColorSubItemCount; i ++)
	{
		pid->clrSub16ItemsText[i] = ::GetSysColor(COLOR_WINDOWTEXT);
	}
...
BOOL CReportCtrl::_AssignNewItemData(int iItem, const CString* pTexts, int nSize)
{
	...
	for (i = 0; i < min(nSize, GetColumnCount()); i++)
...
void CReportCtrl::OnCustomDraw(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = CDRF_DODEFAULT;
	...
	else if (pLVCD->nmcd.dwDrawStage == (CDDS_ITEMPREPAINT | CDDS_SUBITEM))
	{
		...
		ItemData *pid = (ItemData *) pLVCD->nmcd.lItemlParam;
		if (pid)
		{
			if (TRUE == pid->bUseUserClr)
			{
				pLVCD->clrText = pid->clrText;
				if (::GetSysColor(COLOR_WINDOWTEXT) == pid->clrText)
				{
					if (pLVCD->iSubItem >= 0 && pLVCD->iSubItem < GetColumnCount())
					{
						pLVCD->clrText = pid->clrSub16ItemsText[pLVCD->iSubItem];
					}
				}
				pLVCD->clrTextBk = pid->clrBk;
...
void CReportCtrl::SetItemTextColor(int nItem, COLORREF clrText)
{
	ItemData* pid = reinterpret_cast<ItemData*>(CListCtrl::GetItemData(nItem));
	ASSERT(pid);
	pid->bUseUserClr = TRUE;
	pid->clrText = clrText;
}

void CReportCtrl::SetSubItemTextColor(int nItem, int nSubItem, COLORREF clrText)
{
	ItemData* pid = reinterpret_cast<ItemData*>(CListCtrl::GetItemData(nItem));
	ASSERT(pid);
	pid->bUseUserClr = TRUE;
	pid->clrSub16ItemsText[nSubItem] = clrText;
}

void CReportCtrl::SetItemBkColor(int nItem, COLORREF clrBk)
{
	ItemData* pid = reinterpret_cast<ItemData*>(CListCtrl::GetItemData(nItem));
	ASSERT(pid);
	pid->bUseUserClr = TRUE;
...
COLORREF CReportCtrl::GetItemTextColor(int nItem)
{
	ItemData* pid = reinterpret_cast<ItemData*>(CListCtrl::GetItemData(nItem));
	ASSERT(pid);
...
COLORREF CReportCtrl::GetSubItemTextColor(int nItem, int nSubItem)
{
	ItemData* pid = reinterpret_cast<ItemData*>(CListCtrl::GetItemData(nItem));
	ASSERT(pid);
	...
	return pid->clrSub16ItemsText[nSubItem];
}

COLORREF CReportCtrl::GetItemBkColor(int nItem)
{
	ItemData* pid = reinterpret_cast<ItemData*>(CListCtrl::GetItemData(nItem));
	ASSERT(pid);
...
void CReportCtrl::ClearItemColor(int nItem)
{
	ItemData* pid = reinterpret_cast<ItemData*>(CListCtrl::GetItemData(nItem));
	ASSERT(pid);
	pid->bUseUserClr = FALSE;
...
// added by FanZF 20071101
void CReportCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
// 	int iItem =HitTest(point, &m_uFlags);
// 	if (-1 != iItem)
// 	{
// 		CRect rc;
...
// 		if (1)
// 		{
// 			UINT nState = GetItemState( iItem, TVIS_STATEIMAGEMASK ) >> 12;
// 			nState=(nState==3)?1:3;
// 			SetItemState( iItem, INDEXTOSTATEIMAGEMASK(nState), TVIS_STATEIMAGEMASK );
// 		}
// 	}
