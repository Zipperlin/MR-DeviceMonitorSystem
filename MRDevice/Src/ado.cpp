...
BOOL CADORecordset::GetFieldInfo(FieldPtr pField, CADOFieldInfo* fldInfo)
{
	...
	wcscpy((wchar_t *)fldInfo->m_strName, (LPCTSTR)pField->GetName());
...
void CADORecordset::dump_com_error(_com_error &e)
{
	...
	_bstr_t bstrSource(e.Source());
	_bstr_t bstrDescription(e.Description());
	/* Originality source code
	ErrorStr.Format( _T("CADORecordset Error\n\tCode = %08lx\n\tCode meaning = %s\n\tSource = %s\n\tDescription = %s\n"),
		e.Error(), e.ErrorMessage(), (LPCSTR)bstrSource, (LPCSTR)bstrDescription );
	*/
	CString szDescription(_T("")), szSource(_T(""));
	szSource = _com_util::ConvertBSTRToString(bstrSource);
	szDescription = _com_util::ConvertBSTRToString(bstrDescription);
    
	ErrorStr.Format( _T("CADORecordset Error\n\tCode = %08lx\n\tCode meaning = %s\n\tSource = %s\n\tDescription = %s\n"),
		             e.Error(), e.ErrorMessage(), szSource, szDescription);
